...
def write_summary_comparisons(writer, comparisons):
    ...
    writer.write_data_frame(storage_by_cat, COMPARISONS_SHEET, STORAGE_CAT_INDEX)
    writer.write_data_frame(storage_by_group, COMPARISONS_SHEET, STORAGE_GROUP_INDEX)
    writer.write_data_frame(compute, COMPARISONS_SHEET, SERVICE_INDEX)
...
def write_storage_summary(writer, summary_date, storage_summary):
    storage_by_cat = storage_summary.by_category[['Size', 'Buffer', 'Total', 'Group']]  # select columns
